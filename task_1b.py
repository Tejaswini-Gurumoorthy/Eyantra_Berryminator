...
def init_remote_api_server():
	...
	##############	ADD YOUR CODE HERE	##############
	sim.simxFinish(-1) # jclose all opened connections
...
def get_vision_sensor_image(client_id):
	...
	##############	ADD YOUR CODE HERE	##############
	vision = sim.simxGetObjectHandle('vision_sensor')
	return_code,image_resolution,vision_sensor_image= sim.simxGetVisionSensorImage(client_id,vision,0,sim.simx_opmode_streaming)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	numpy_array= np.array(vision_sensor_image,np.uint8)
	numpy_array_new = np.reshape(image_resolution[2],image_resolution[0],image_resolution[1])
	new_coloured_array= cv2.cvtColor(numpy_array_new,cv2.COLOR_BGR2RGB)
	transformed_image= np.flip(new_coloured_array,axis=0)
